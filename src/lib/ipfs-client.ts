...
export class IPFSClient {
  ...
  private constructor() {
    this.apiKey = process.env.NEXT_PUBLIC_IPFS_API_KEY || '';
    this.apiUrl = process.env.NEXT_PUBLIC_IPFS_API_URL || 'http://52.38.175.117:5000';
    if (!this.apiKey) {
      console.warn('IPFS API key not found');
    }
  }
  ...
  async uploadFile(options: UploadOptions): Promise<any> {
    try {
      ...
      if (!this.apiKey) {
        throw new Error('IPFS API key is not configured');
      }
      ...
      try {
        ...
        console.debug('Upload response:', output);

        if (!output.Hash) {
          throw new Error('Invalid response from IPFS: ' + JSON.stringify(output));
        }
        ...
        console.error('Error during IPFS upload:', uploadError);
        if (uploadError instanceof Error) {
          throw new Error(`IPFS upload failed: ${uploadError.message}`);
        }
        throw uploadError;
      }
    } catch (error) {
